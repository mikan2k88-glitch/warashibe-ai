...
CODEX_MILESTONE_EXECUTION_GATE_VERSION = "0.2"
...
def evaluate_codex_execution_gate(explicit_human_approval=False):
    ...
    safe_contract = (
        contract.get("allowed_branches") == ("research-lab", "main")
        ...
        and contract.get("main_code_changes_allowed") is True
        and contract.get("main_branch_write_requires_human_gate") is True
...
def validate_codex_execution_gate():
    ...
    assert closed["allowed_branches"] == ("research-lab", "main")
    assert closed["repository_code_scope"] == "all_repository_code"
    ...
    assert closed["main_code_changes_allowed"] is True
    assert closed["main_branch_write_requires_human_gate"] is True
