...
EBAY_BROWSE_ADAPTER_VERSION = "0.2"
...
def _money(value):
    ...
    if not math.isfinite(amount) or amount < 0:
        raise ValueError("listing price must be a finite non-negative number")
...
def browse_search_to_records(payload, *, observed_at=None):
    ...
    if not isinstance(summaries, list):
        raise TypeError("itemSummaries must be a list")
    observed_at = observed_at or datetime.now(timezone.utc).isoformat()
    records, rejected = [], []
    for index, item in enumerate(summaries):
        try:
            if not isinstance(item, dict):
                raise TypeError("item summary must be a dictionary")
            ...
            if not isinstance(categories, list) or not categories or not isinstance(categories[0], dict):
                raise ValueError("categories must be a list of dictionaries")
            category = str(categories[0].get("categoryName") or "unknown")
            records.append({
                "external_id": item_id,
                "name": title,
                "category": category,
